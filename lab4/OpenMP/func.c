...
#pragma omp parallel for num_threads(NUM) private(j, index_X, index_Y)
   	for(i = 0; i<n; i++)
	  {
   		for(j = 0; j < Ones; j++)
		  {
...
#pragma omp parallel for num_threads(NUM) private(j)
	for (i = 0; i<n; i++)
	  {
               probability[i] = 0;
   		for(j = 0; j < Ones; j++) 
...
#pragma omp parallel for num_threads(NUM) reduction(+:sumWeights)
   	for(i = 0; i < n; i++)
...
void func3(double *arrayX, double *arrayY, double *weights, double *x_e, double *y_e, int n)
{
	...
	int i;
#pragma omp parallel for num_threads(NUM) reduction(+: estimate_x,estimate_y)
	for(i = 0; i < n; i++){
